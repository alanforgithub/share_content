...
import com.ntian.listener.OnPageChangeListener;
import com.ntian.model.OptionButton;
import com.ntian.utils.ToastUtil;
import com.ntian.view.MobileFrameLayout;
...
public class Camera extends ActivityBase implements
		...
		CameraScreenNail.FrameListener, OnPageChangeListener, OnClickListener,
		OnCurrentModeListener {
	...
	public void onCreate(Bundle icicle) {
		...
		mFramelayout = (MobileFrameLayout) findViewById(R.id.layout_root);
		...
		mFramelayout.setOnPageChangeListener(this);
	}
	...
	private void initializeCommonManagers() {
		...
		mPickerManager.setOnCurrentModeListener(this);
	...
	private void initializeForOpeningProcess() {
		...
		mViewLayerNormal = (ViewGroup) findViewById(R.id.view_layer_normal); // 除了拍照和录像按钮之外所有的view
		mViewLayerTop = (ViewGroup) findViewById(R.id.view_layer_top);
		mViewLayerShutter = (ViewGroup) findViewById(R.id.view_layer_shutter); // 拍照和录像按钮
	...
	private ModePicker.OnModeChangedListener mModeChangedListener = new ModePicker.OnModeChangedListener() {
		...
		public void onModeChanged(int newMode) {
			...
			if (mCameraActor.getMode() != newMode) {
				...
				switch (newMode) {
				case ModePicker.MODE_PHOTO: // 普通模式
					mCameraActor = new PhotoActor(Camera.this);
					break;
				case ModePicker.MODE_HDR: // HDR模式
					mCameraActor = new HdrActor(Camera.this);
					break;
				case ModePicker.MODE_FACE_BEAUTY: // 美颜模式
					mCameraActor = new FaceBeautyActor(Camera.this);
					break;
				case ModePicker.MODE_PANORAMA: // 全景模式
					mCameraActor = new PanoramaActor(Camera.this);
					break;
				case ModePicker.MODE_MAV: // 多角度模式
				...
				case ModePicker.MODE_SMILE_SHOT:
					...
					break;
				case ModePicker.MODE_MOTION_TRACK: // 追踪模式
				...
				case ModePicker.MODE_VIDEO:
					...
					break;
				case ModePicker.MODE_LIVE_PHOTO: // 情景模式
				...
				currentCameraMode = mCameraActor.getMode();
	...
	private PickerManager.PickerListener mPickerListener = new PickerManager.PickerListener() {
		...
		@Override
		public boolean onFlashPicked(final String flashMode) {
	...
	public void setFlashIcon(String mode) {
		if ("auto".equals(mode)) {
			flashBtn.setBackgroundResource(R.drawable.ntian_ic_flash_button_auto);
		} else if ("off".equals(mode)) {
			flashBtn.setBackgroundResource(R.drawable.ntian_ic_flash_button_off);

		} else if ("on".equals(mode)) {
			flashBtn.setBackgroundResource(R.drawable.ntian_ic_flash_button_on);
	...
	private void initView() {
		...
		getOptionItemInfos();
		mNtianGridViewAdapter = new NtianGridViewAdapter(Camera.this,
				optionButtonList);
		mOptionGridView.setAdapter(mNtianGridViewAdapter);

		mNtianGridViewAdapter
				.setOnOptionItemClickListener(new OnOptionItemClickListener() {

					@Override
					public void optionClick(OptionButton button) {

						selectCameraFunc(button);

					}
				});
		findViewById(R.id.currentPage).setVisibility(View.VISIBLE);
		settingButton = (ImageButton) findViewById(R.id.setting_button_icon);
		settingButton.setOnClickListener(this);
		findViewById(R.id.setting_button_icon_right).setOnClickListener(this);
		radioGroup = (RadioGroup) findViewById(R.id.currentPage);
		leftPoint = (RadioButton) findViewById(R.id.left_point);
		middlePoint = (RadioButton) findViewById(R.id.middle_point);
		rightPoint = (RadioButton) findViewById(R.id.right_point);
		flashBtn = (ImageButton) findViewById(R.id.flashlightButton);
		flashBtn.setOnClickListener(this);
		flashBtn.setVisibility(View.VISIBLE);
		backFrontEyeBtn = (ImageButton) findViewById(R.id.back_front_eye);
		backFrontEyeBtn.setOnClickListener(this);
		backFrontEyeBtn.setVisibility(View.VISIBLE);
	 ...
	 * 隐藏后添加的view
	 */
	public void hideNtianView() {
		backFrontEyeBtn.setVisibility(View.GONE);
	 ...
	 * @param button
	 */
	private void selectCameraFunc(OptionButton button) {

		if (button.mSwitch) {
			currentCameraMode = ModePicker.MODE_PHOTO;
			if (null != mModePicker) {
				if ("HDR".equals(button.optionName)) { // HDR模式和其他模式不一致，需要单独处理
					if (null != mPickerManager) {
						mPickerManager.switchDHR();
					}
				} else {
					gotoNormalMode();
				}
		...
		if (null != button) {
			...
			case 1: // 全景
				if (null != mModePicker) {
					mModePicker.nTianSelectMode(ModePicker.MODE_PANORAMA);
				}
				break;
			case 2: // 美颜
				if (null != mModePicker) {
					mModePicker.nTianSelectMode(ModePicker.MODE_FACE_BEAUTY);
				}
				break;
			case 3: // 声控相机
				break;
	...
	private void gotoNormalMode() {
		if (null != mModePicker)
			mModePicker.nTianSelectMode(ModePicker.MODE_PHOTO);
	...
	private void setCurrentCameraMode() {
		...
		for (OptionButton option : optionButtonList) {
			option.mSwitch = false;
		}

		switch (currentCameraMode) {
		case ModePicker.MODE_PANORAMA:// ModePicker.MODE_PANORAMA: // 全景
			optionButtonList.get(0).mSwitch = true;
			showCloseBtn(getString(R.string.close_panorama));
			break;
		case ModePicker.MODE_FACE_BEAUTY: // 美颜
			optionButtonList.get(1).mSwitch = true;
			showCloseBtn(getString(R.string.close_beauty));
			break;
		case ModePicker.MODE_HDR:// HDR
			optionButtonList.get(3).mSwitch = true;
			break;
		// case 3: // 声控相机
		// showCloseBtn(getString(R.string.close_beauty));
		// //TODO
		...
		mNtianGridViewAdapter.notifyDataSetChanged();
	}
	 ...
	 * @param mode
	 */
	private void showCloseBtn(String mode) {
		closeButton.setText(mode);
	...
	int[] selectOptionIcons = new int[] {
			R.drawable.ntian_ic_indicator_panorama_selected,
			R.drawable.ntian_ic_indicator_facebeauty_selected,
			R.drawable.ntian_ic_indicator_audio_capture_selected,
			R.drawable.ntian_ic_indicator_hdr_selected,
			R.drawable.ntian_ic_indicator_burst_shoot_selected,
			R.drawable.ntian_ic_indicator_hand_night_selected,
			R.drawable.ntian_ic_indicator_manual_selected };
	int[] optionItemID = new int[] { 1, 2, 3, 4, 5, 6, 7 };

	/** 设置按钮 */
	private ImageButton settingButton;
	private RadioButton leftPoint;
	private RadioButton middlePoint;
	private RadioButton rightPoint;
	private ImageButton flashBtn;
	private ImageButton backFrontEyeBtn;
	/** 当前相机的模式 */
	...
	private void getOptionItemInfos() {
		...
		for (int i = 0; i < optionNames.length; i++) {
			...
			option.optionName = getString(optionNames[i]);
			option.drawableId = optionIcons[i];
			option.optionId = optionItemID[i];
			option.selectDrawableId = selectOptionIcons[i];
	...
	public void onPageChange(int currentPage) {
		switch (currentPage) {
		case MobileFrameLayout.LEFT:
			leftPoint.setChecked(true);
			flashBtn.setVisibility(View.GONE);
			backFrontEyeBtn.setVisibility(View.GONE);
			break;
		case MobileFrameLayout.MIDDLE:
			middlePoint.setChecked(true);
			flashBtn.setVisibility(View.VISIBLE);
			backFrontEyeBtn.setVisibility(View.VISIBLE);
			break;
		case MobileFrameLayout.RIGHT:
			rightPoint.setChecked(true);
			flashBtn.setVisibility(View.GONE);
			backFrontEyeBtn.setVisibility(View.GONE);
	...
	public void onClick(View view) {
		...
		switch (view.getId()) {
		case R.id.setting_button_icon: // 设置按钮
		case R.id.setting_button_icon_right: // 设置按钮
			// TODO
			ToastUtil.showToast(getApplicationContext(), "设置按钮");
			break;

		case R.id.flashlightButton: // 闪光灯
			if (null != mPickerManager) {
				mPickerManager.switchFlashMode();
			}
			break;
		case R.id.back_front_eye: // 前后摄像头
			// onPreviewTextureCopied();
			if (null != mPickerManager) {
				mPickerManager.switchCamera();
			}
			break;
		case R.id.closeButton: // 关闭其他模式，进入普通模式
			gotoNormalMode();
			closeButton.setVisibility(View.GONE);
			break;
	...
	private MobileFrameLayout mFramelayout;
	private NtianGridViewAdapter mNtianGridViewAdapter;
	...
	public void currentMode(Object type, Object mode) {
		...
		switch (mType) {
		case OnCurrentModeListener.FLASH_MODE:
			setFlashIcon((String) mode);
			break;
		case OnCurrentModeListener.HDR_SWITCH:
			OptionButton button = optionButtonList.get(3);
			if ("on".equals((String) mode)) {
				button.mSwitch = true;
			} else {
				button.mSwitch = false;
			}
