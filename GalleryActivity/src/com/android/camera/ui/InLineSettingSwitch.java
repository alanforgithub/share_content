...
public class InLineSettingSwitch extends InLineSettingItem 
    ...
    OnCheckedChangeListener mCheckedChangeListener = new OnCheckedChangeListener() {
        ...
        public void onCheckedChanged(CompoundButton buttonView, boolean desiredState) {
        	...
        	if (desiredState && mPreference.getKey().equals(CameraSettings.KEY_RECORD_LOCATION)) {
        		if (isLocationEnabled()) {
    				changeIndex(1);
    			} else {
    				 new AlertDialog.Builder(mContext)
    	                .setTitle(mContext.getResources().getString(R.string.location_title))
    	                .setMessage(mContext.getResources().getString(R.string.location_message))
    	                .setPositiveButton(mContext.getResources().getString(R.string.location_settings), new DialogInterface.OnClickListener() {
    	                    public void onClick(DialogInterface dialog, int whichButton) {
    	                    	Intent intent = new Intent(android.provider.Settings.ACTION_LOCATION_SOURCE_SETTINGS);
    	                    	mContext.getSettingManager().collapse(true);
    	                    	mContext.startActivityForResult(intent,200);
    	                    }
    	                })
    	                .setNegativeButton(mContext.getResources().getString(R.string.location_cancel), new DialogInterface.OnClickListener() {
    	                    public void onClick(DialogInterface dialog, int whichButton) {
