...
public class ModePicker extends ViewManager implements View.OnClickListener,
	...
	public static final int MODE_PHOTO_SGINLE_3D = OFFSET_STEREO_SINGLE
			+ MODE_PHOTO;
	public static final int MODE_PANORAMA_SINGLE_3D = OFFSET_STEREO_SINGLE
			+ MODE_PANORAMA;
	...
	private void highlightCurrentMode() {
		...
		for (int i = 0; i < MODE_NUM_ALL; i++) {
			if (mModeViews[i] != null) {
				if (i == index) {
					//mModeViews[i].setImageResource(MODE_ICONS_HIGHTLIGHT[i]);  //modify by wgb  hide top icon
				} else {
					//mModeViews[i].setImageResource(MODE_ICONS_NORMAL[i]);     //modify by wgb   hide top icon
				}
			}
			if (MODE_HDR == index || MODE_SMILE_SHOT == index
					|| MODE_ASD == index || MODE_GESTURE_SHOT == index) {
				//mModeViews[MODE_PHOTO]
				//		.setImageResource(MODE_ICONS_HIGHTLIGHT[MODE_PHOTO]);   //modify by wgb   hide top icon
			}
		}
	...
	protected View getView() {
		...
		mScrollView = (ModePickerScrollView) view
				.findViewById(R.id.mode_picker_scroller);
		mModeViews[MODE_PHOTO] = (RotateImageView) view
				.findViewById(R.id.mode_photo);
		mModeViews[MODE_LIVE_PHOTO] = (RotateImageView) view
				.findViewById(R.id.mode_live_photo);
		mModeViews[MODE_MOTION_TRACK] = (RotateImageView) view
				.findViewById(R.id.mode_motion_track);
		mModeViews[MODE_FACE_BEAUTY] = (RotateImageView) view
				.findViewById(R.id.mode_face_beauty);
		mModeViews[MODE_PANORAMA] = (RotateImageView) view
				.findViewById(R.id.mode_panorama);
		mModeViews[MODE_MAV] = (RotateImageView) view
				.findViewById(R.id.mode_mav);
		
		//modify by wgb hide top icon
		mScrollView.setVisibility(View.GONE);
		mScrollView.setClickable(false);
		mScrollView.setEnabled(false);
		mScrollView.setFocusable(false);
		mModeViews[MODE_PHOTO].setVisibility(View.GONE);
		mModeViews[MODE_PHOTO].setClickable(false);
		mModeViews[MODE_PHOTO].setEnabled(false);
		mModeViews[MODE_PHOTO].setFocusable(false);
		mModeViews[MODE_LIVE_PHOTO].setVisibility(View.GONE);
		mModeViews[MODE_LIVE_PHOTO].setClickable(false);
		mModeViews[MODE_LIVE_PHOTO].setEnabled(false);
		mModeViews[MODE_LIVE_PHOTO].setFocusable(false);
		mModeViews[MODE_MOTION_TRACK].setVisibility(View.GONE);
		mModeViews[MODE_MOTION_TRACK].setClickable(false);
		mModeViews[MODE_MOTION_TRACK].setEnabled(false);
		mModeViews[MODE_MOTION_TRACK].setFocusable(false);
		mModeViews[MODE_FACE_BEAUTY].setVisibility(View.GONE);
		mModeViews[MODE_FACE_BEAUTY].setClickable(false);
		mModeViews[MODE_FACE_BEAUTY].setEnabled(false);
		mModeViews[MODE_FACE_BEAUTY].setFocusable(false);
		mModeViews[MODE_PANORAMA].setVisibility(View.GONE);
		mModeViews[MODE_PANORAMA].setClickable(false);
		mModeViews[MODE_PANORAMA].setEnabled(false);
		mModeViews[MODE_PANORAMA].setFocusable(false);
		mModeViews[MODE_MAV].setVisibility(View.GONE);
		mModeViews[MODE_MAV].setClickable(false);
		mModeViews[MODE_MAV].setEnabled(false);
		mModeViews[MODE_MAV].setFocusable(false);
	...
	private void applyListener() {
		for (int i = 0; i < MODE_NUM_ALL; i++) {
			if (mModeViews[i] != null) {
				...
				mModeViews[i].setVisibility(View.GONE);
			}
		}
	}
	...
	/**
	 * 模式选择
	 * 0普通模式，2美颜模式，3全景模式，4多角度模式，7跟踪模式，9情景模式
	 * @param mode
	 */
	public void nTianSelectMode(int mode) {
		setEnabled(false);
	...
	public void onClick(View view) {
		...
		//TODO modify by wgb , refuse the origin click event
...
//			if (mModeToast == null) {
//				mModeToast = OnScreenToast.makeText(getContext(),
...
//		}
	...
	public void onRefresh() {
		...
		if (supportModes < MODE_MIN_COUNTS && supportModes > 1) {
			mModeMarginBottom = (mDisplayWidth - supportModes * mModeWidth)
					/ (supportModes - 1);
		}
		...
		for (int i = 0; i < MODE_NUM_ALL; i++) {
			if (mModeViews[i] != null) {
				boolean visible = ModeChecker.getModePickerVisible(
						getContext(), getContext().getCameraId(), i);

				// TODO modify by wgb
				mModeViews[i].setVisibility(visible ? View.VISIBLE : View.GONE);
				mModeViews[i].setLayoutParams(mLayoutParams);
				mModeViews[i].setPadding(MODE_DEFAULT_PADDING,
						MODE_DEFAULT_PADDING, MODE_DEFAULT_PADDING,
						MODE_DEFAULT_PADDING);
				if (visible) {
					visibleCount++;
				}
			}
		...
		for (int i = MODE_ICON_ORDER.length - 1; i >= 0; i--) {
			...
			if (mModeViews[index] != null
					&& mModeViews[index].getVisibility() == View.VISIBLE) {
	...
	private int getModeWidth() {
		Bitmap bitmap = BitmapFactory.decodeResource(getContext()
				.getResources(), MODE_ICONS_NORMAL[MODE_PHOTO]);
	...
	private int getDefaultMarginBottom() {
		...
		return (mDisplayWidth - MODE_MIN_COUNTS * mModeWidth)
				/ (MODE_MIN_COUNTS - 1)
