...
public class PhotoView extends GLView {
    ...
    private static final boolean PLAY_OPEN_ANIMATION = false;
    ...
    protected void onLayout(
        ...
        if (changeSize) {
            if (PLAY_OPEN_ANIMATION && !mModel.isCamera(mModel.getCurrentIndex())) {
                int startW = Math.min(8, right - left);
                int startH = Math.min(8, bottom - top);
                int offsetX = (right - left - startW) / 2;
                int offsetY = (bottom - top - startH) / 2;
                if (startW > 0 && startH > 0) {
                    Rect openAnimationRect = new Rect(
                            left + offsetX, 
                            top + offsetY, 
                            right - offsetX, 
                            bottom - offsetY);
                    mPositionController.setOpenAnimationRect(openAnimationRect);
                }
            }
...
*/
        ...
        public boolean onScaleBegin(float focusX, float focusY) {
            ...
            if (mIgnoreScaleGesture) return true;
            if (mIgnoreSwipingGesture) return true;
            // scale state error, ignore this scale gesture
            if (mModeChanged) return true;
            // M: 6592 panorama add @{
            if (MediatekFeature.isPanorama3DSupported() && mTileView.getIsPanorama() && !mFilmMode
                    && mPanoramaMode == PANORAMA_SHOW_MODE_3D) {
    ...
    public void enterCameraPreview() {
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                mListener.lockOrientationSpecific(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
