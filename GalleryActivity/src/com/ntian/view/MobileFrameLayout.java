...
public class MobileFrameLayout extends LinearLayout {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (ev.getAction() == MotionEvent.ACTION_DOWN) {
			...
			if (currentPage == LEFT
					&& Math.abs(xDown - xMove) < Math.abs(downY - moveY)) {
	...
	private void scrollToLeft() {
		...
		if(null != mOnPageChangeListener){
			mOnPageChangeListener.onPageChange(LEFT);
		}
	}
	 ...
	 */
	public void scrollToMiddle(int speed) {
		new ScrollTask().execute(speed);
		if(null != mOnPageChangeListener){
			mOnPageChangeListener.onPageChange(MIDDLE);
		}
	}
	...
	private void scrollToRight() {
		new ScrollTask().execute(-30);
		if(null != mOnPageChangeListener){
			mOnPageChangeListener.onPageChange(RIGHT);
		}
	}
	...
	class ScrollTask extends AsyncTask<Integer, Integer, Integer> {
		...
		protected void onProgressUpdate(Integer... leftMargin) {
			...
			Log.i("wgb", "leftMargin2 = " + leftMargin[0]);
